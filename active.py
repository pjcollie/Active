...
class RetirementTrackerApp:
    ...
    def create_gui(self):
        ...
        self.tree.heading("Eligible", text="Retirement Eligible")
        ...
        self.actual_tree.heading("ID", text="ID")
        self.actual_tree.heading("Name", text="Name")
        self.actual_tree.heading("Department", text="Department")
        self.actual_tree.heading("Total Income", text="Total Income")
        self.actual_tree.heading("Contribution", text="Contribution")
        self.actual_tree.heading("Eligible", text="Eligible")
        ...
        self.hypothetical_tree.heading("ID", text="ID")
        self.hypothetical_tree.heading("Name", text="Name")
        self.hypothetical_tree.heading("Department", text="Department")
        self.hypothetical_tree.heading("Total Income", text="Total Income")
        self.hypothetical_tree.heading("Contribution", text="Contribution")
        self.hypothetical_tree.heading("Eligible", text="Eligible")
    ...
    def refresh_summary(self):
        ...
        for row in rows:
            ...
            self.tree.insert("", tk.END, values=(row[0], blurred_name, row[2], f"${row[3]:.2f}", f"${contribution:.2f}", eligible_status))
            total_income += row[3]
            if row[4]:
                total_contribution += contribution
        total_spend = total_income + total_contribution - total_profit
        self.total_income_label.config(text=f"{period_label} Total Income: ${total_income:.2f}")
        self.total_contribution_label.config(text=f"{period_label} Total Contribution: ${total_contribution:.2f}")
        self.total_taxable_income_label.config(text=f"{period_label} Total Taxable Income: ${total_profit:.2f}")
        self.total_spend_label.config(text=f"{period_label} Total Spend: ${total_spend:.2f}")
    ...
    def edit_hypothetical(self, event):
        ...
        if col_index == 5:  # Eligible column
            ...
            entry.insert(0, item["values"][col_index].replace("$", ""))
        def save_edit():
            try:
                if col_index == 3:  # Total Income
                    ...
                    if new_value < 0:
                        raise ValueError
                    eligible = self.hypothetical_data.get(employee_id, [0, 0, False])[2]
                    income = new_value / (self.contribution_percentage / 100) if eligible else self.hypothetical_data.get(employee_id, [0, 0, False])[0]
                    self.hypothetical_data[employee_id] = (income, new_value, eligible)
                elif col_index == 5:  # Eligible
                    new_value = eligible_var.get() == "Yes"
                    income = self.hypothetical_data.get(employee_id, [0, 0, SQL])
    ...
    def refresh_scenarios(self):
        ...
        for row in self.cursor.fetchall():
            ...
            self.actual_tree.insert("", tk.END, values=(employee_id, blurred_name, department, f"${total_income:.2f}", f"${contribution:.2f}", eligible_status))
        ...
        for employee_id, (total_income, contribution, eligible) in self.hypothetical_data.items():
            ...
            if result:
                ...
                blurred_name = self.blur_name(name)
                eligible_status = "Yes" if eligible else "No"
                self.hypothetical_tree.insert("", tk.END, values=(employee_id, blurred_name, department, f"${total_income:.2f}", f"${contribution:.2f}", eligible_status))
        ...
        for employee_id, (hyp_income, hyp_contribution, hyp_eligible) in self.hypothetical_data.items():
            if employee_id in actual_data:
                ...
                if hyp_income != act_income:
                    differences.append(f"{blurred_name}: Change Total Income from ${act_income:.2f} to ${hyp_income:.2f}")
                if hyp_contribution != act_contribution:
                    differences.append(f"{blurred_name}: Change Contribution from ${act_contribution:.2f} to ${hyp_contribution:.2f}")
    ...
    def refresh_compare(self):
        ...
        self.employee1_info.config(text=f"Name: {self.blur_name(name1)}\n"
                                      f"Department: {dept1}\n"
                                      f"Total Income: ${income1:.2f}\n"
                                      f"Contribution: ${contrib1:.2f}\n"
        ...
        self.employee2_info.config(text=f"Name: {self.blur_name(name2)}\n"
                                      f"Department: {dept2}\n"
                                      f"Total Income: ${income2:.2f}\n"
                                      f"Contribution: ${contrib2:.2f}\n"
